...
namespace ObjectComparer.Tests.Tests
{
    ...
    {
        private const string TYPE_NAME = "long";
        [Test]
        public void Test_Default()
        {
            ...
            copy.TestLong = 1;
            TestContext.Out.WriteLine("copy {0}: {1}", TYPE_NAME, copy.TestLong);
            TestContext.Out.WriteLine("model {0}: {1}", TYPE_NAME, model.TestLong);
        ...
        public void Test_NullableStartsWithNull()
        {
            ...
            // Act
            copy.TestLongNullable = 1;

            // Assert
            TestContext.Out.WriteLine("copy {0}?: {1}", TYPE_NAME, copy.TestLongNullable?.ToString() ?? "<NULL>");
            TestContext.Out.WriteLine("model {0}?: {1}", TYPE_NAME, model.TestLongNullable?.ToString() ?? "<NULL>");
            TestContext.Out.WriteLine("copy HasBeenModified: {0}", model.HasBeenModified(copy));
            Assert.IsTrue(model.HasBeenModified(copy), "Change {0}? has not been registered", TYPE_NAME);
