...
namespace ObjectComparer.Tests.Tests
{
    ...
    {
        private const string TYPE_NAME = "ushort";
        [Test]
        ...
        public void Test_NullableStartsWithNull()
        {
            ...
            // Act
            copy.TestUShortNullable = 1;

            // Assert
            TestContext.Out.WriteLine("copy {0}?: {1}", TYPE_NAME, copy.TestUShortNullable?.ToString() ?? "<NULL>");
            TestContext.Out.WriteLine("model {0}?: {1}", TYPE_NAME, model.TestUShortNullable?.ToString() ?? "<NULL>");
