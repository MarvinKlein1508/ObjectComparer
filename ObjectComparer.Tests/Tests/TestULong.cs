...
namespace ObjectComparer.Tests.Tests
{
    ...
    {
        private const string TYPE_NAME = "ulong";
        [Test]
        ...
        public void Test_NullableStartsWithNull()
        {
            ...
            // Act
            copy.TestULongNullable = 1;

            // Assert
            TestContext.Out.WriteLine("copy {0}?: {1}", TYPE_NAME, copy.TestULongNullable?.ToString() ?? "<NULL>");
            TestContext.Out.WriteLine("model {0}?: {1}", TYPE_NAME, model.TestULongNullable?.ToString() ?? "<NULL>");
