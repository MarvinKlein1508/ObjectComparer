...
namespace ObjectComparer.Tests.Tests
{
    ...
    {
        private const string TYPE_NAME = "uint";
        [Test]
        ...
        public void Test_NullableStartsWithNull()
        {
            ...
            // Act
            copy.TestUIntNullable = 1;

            // Assert
            TestContext.Out.WriteLine("copy {0}?: {1}", TYPE_NAME, copy.TestUIntNullable?.ToString() ?? "<NULL>");
            TestContext.Out.WriteLine("model {0}?: {1}", TYPE_NAME, model.TestUIntNullable?.ToString() ?? "<NULL>");
