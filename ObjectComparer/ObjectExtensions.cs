        ...
        public static bool HasBeenModified<T>(this T original, T vergleich, bool innerLoop = false)
        {
            ...
            else
            {
                foreach (PropertyInfo prop in original.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public))
                {
                    ...
                    Type vergleichType = vergleichValue.GetType();
                    ...
                    else if (originalType.IsEnum && vergleichType.IsEnum)
                    {
                    ...
                    else
                    {
                        ...
                        if (prop.PropertyType.IsPrimitive || originalValue is string or decimal or float or double or char or byte or bool or Guid or int or long or nint or nuint or sbyte or short or uint or ulong or ushort or TimeOnly)
                        {
                            if (!originalValue.Equals(vergleichValue))
                            {
                                return true;
